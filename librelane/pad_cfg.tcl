...
foreach side $sides {
    ...
    foreach inst_name $::env($side) {
        ...
        if {$side == "PAD_EAST"} {
            set break_pos [expr $cur_pos + $width]
            if {$counter == 7} {
                place_pad -row [dict get $row_names $side] -location $break_pos "break_1" -master "gf180mcu_fd_io__brk5"
            }
            if {$counter == 12} {
                place_pad -row [dict get $row_names $side] -location $break_pos "break_2" -master "gf180mcu_fd_io__brk5"
            }
        }
